...
class IndexMgr {
    ...
    */
   public void createIndex(String idxname, String tblname, String fldname, IndexType type, Transaction tx) {
      ...
      ts.setString("indextype", type.toString());
      ts.close();
   }
   ...
   public Map<String, IndexInfo> getIndexInfo(String tblname, Transaction tx) {
      ...
      while (ts.next())
         if (ts.getString("tablename").equals(tblname)) {
            ...
            IndexType type = IndexType.getType(ts.getString("indextype"));
            Layout tblLayout = tblmgr.getLayout(tblname, tx);
            StatInfo tblsi = statmgr.getStatInfo(tblname, tblLayout, tx);
            IndexInfo ii = new IndexInfo(idxname, fldname, type, tblLayout.schema(), tx, tblsi);
