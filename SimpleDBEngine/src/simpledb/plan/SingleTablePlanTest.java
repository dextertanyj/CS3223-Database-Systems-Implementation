...
public class SingleTablePlanTest {
	public static void main(String[] args) throws Exception {
		...
		Term t = new Term(Operator.EQ, new Expression("majorid"), new Expression(new Constant(10)));
		Predicate pred = new Predicate(t);
		Plan p2 = new SelectPlan(p1, pred);

		// the Select node for "gradyear = 2020"
		Term t2 = new Term(Operator.EQ, new Expression("gradyear"), new Expression(new Constant(2020)));
		...
		// Changing p2 to p4 will throw an exception because SID is not in the
		// projection list.
		Scan s = p2.open();
		while (s.next())
